...
function checkEligibility(athleteWeights, sledWeight) {
    ...
    let numberOfPersons = athleteWeights.length;

    let totalAthleteWeight = 0;

    athleteWeights.forEach(athleteWeight => {
        totalAthleteWeight += athleteWeight

    });

    let totalTeamWeight = totalAthleteWeight + sledWeight;

    console.log(totalTeamWeight);

    if (numberOfPersons === 1 && sledWeight >= 162) {
        if (totalTeamWeight <= 247) {
            return "Eligible"
        } else {
            return "Not Eligible"
        }
        ...
        if (numberOfPersons === 2 && sledWeight >= 170) {
            if (totalTeamWeight <= 390) {
                return "Eligible"
            } else {
                return "Not Eligible"
            }
            ...
            if (numberOfPersons === 4 && sledWeight >= 210) {
                if (totalTeamWeight <= 630) {
                    return "Eligible"
                } else {
                    return "Not Eligible"
                }
...
checkEligibility([78], 165)
checkEligibility([80], 160)
checkEligibility([80], 170)
checkEligibility([85, 90], 170)
checkEligibility([85, 95], 168)
checkEligibility([112, 97], 185)
checkEligibility([110, 102, 90, 106], 222)
checkEligibility([106, 99, 90, 88], 205)
